...
** Tufte-LaTeXとtufte-org-modeについて                 :tufte:latex:org:mode:
   :PROPERTIES:
...
*** [[https://tufte-latex.github.io/tufte-latex/][Tufte-LaTeX]]
      - [[https://www.edwardtufte.com/tufte/index][Edward R. Tufte]]によって作られたページレイアウト．典型的には文章が左側に配置され，右側には広いマージンがありノート，文献，表，図などが配置されるスタイルである．
...
*** [[https://github.com/tsdye/tufte-org-mode][tufte-org-mode]]
    - [[https://github.com/tsdye/tufte-org-mode/blob/master/README.md][tufte-org-mode]]は，上述のlatexのtufte classをorg-modeから使えるようにした有り難いパッケージである．
*** インストールと設定
    - 下記の2つの設定で使えるようになる．init.orgでの設定の順番はどちらが先でも動く．
**** tufte-org-modeのインストールと設定
     - 以下のようにinit.orgに書き込んで設定する．
     - ox-tufte-latex.elは上記のサイトからダウンロードしてローカルに置いてインストールしている．
       #+begin_src lisp
         ,#+begin_src emacs-lisp
         (quelpa
